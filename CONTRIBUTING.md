# How to Contribute to aptosbot

Hey there, thanks for deciding to contribute to aptosbot. Anything you help with will support the development of aptosbot and will make it better. Let's walk you through the key aspects to ensure your contributions are effective and in harmony with the project's setup.

## Project Structure

aptosbot's design consists of two main domains:
...
- **Backend (root and `src` directory)**: The backend logic is situated in the `src` folder, but the root directory holds the main `package.json` for backend dependency management.
  - All of the focus modes are created using the Meta Search Agent class present in `src/search/metaSearchAgent.ts`. The main logic behind aptosbot lies there.
...
Following these steps will help maintain the integrity of aptosbot's codebase and facilitate a smoother integration of your valuable contributions. Thank you for your support and commitment to improving aptosbot.
